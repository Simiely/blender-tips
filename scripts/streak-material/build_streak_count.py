# build_streak_count.py —— 「滚筒斜纹 · 按条数」（方案 A）
#
# 与「按角度」（方案 B）是【并列的两个独立方案】，各自一份材质 + 一个槽位，复用时选一个跑即可。
#   · A（本脚本）槽 1：直接给「条纹数量 K」「高度条纹数 N」两个整数控件，斜角是算出来的结果
#   · B（build_streak_angle.py）槽 3：「条纹数量 K」+「斜角」，N 由斜角反算
#
...
PANEL_SRC = r'D:/workbuddy/2026-09-20-09-39-42/_liantiao\streak_count_panel.py' 
PANEL_TEXT = 'streak_count_panel.py'

# 控件：(键名, 默认, min, max, soft_min, soft_max, step, precision, 说明)
PROPS = [
    ('条纹数量', 3.0, 1.0, 24.0, 1.0, 12.0, 1, 0,
     '绕柱一周的条纹条数 —— 必须整数，否则接缝处错位'),
    ('高度条纹数', 2.0, 1.0, 24.0, 1.0, 12.0, 1, 0,
     '沿柱高的周期数。与「条纹数量」的比值决定斜角（3:2 ⇒ 41°）。必须整数'),
...
def cleanup_old_ui():
    ...
    for key, cls, txt in (('_streak_count_watch', 'VIEW3D_PT_streak_count', 'streak_count_panel.py'),
...
def rebuild(mat, ctrl):
    ...
    # 高度条纹 N：由「高度条纹数」控件直接驱动（方案 A = 条数优先）
...
def main():
    ...
    try:
        ...
        _wf = bpy.app.driver_namespace.get('_streak_count_watch')
    ...
    n = ctrl.get('高度条纹数') or 1.0
    ang = math.degrees(math.atan((k / n) * H_OVER_P)) if n else 0.0
    print('=== BUILD_OK (方案A · 按条数) ===')
    print('条纹      : K=%g（绕柱） × N=%g（高度）⇒ 斜角 %.1f°' % (k, n, ang))
