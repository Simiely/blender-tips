...
> 📌 **文档基线**:2026-09-11(commit `418fd3f`)v1.15.1 文档清理:残留节/重复节/误插行/失效链接与锚点修复
...
- **按材质拆分 = `bpy.ops.mesh.separate(type='MATERIAL')`(唯一真实算子)**:语义是"按**材质槽**、对**整个网格**生效、**与选择无关**"(官方手册原文)。**原对象保留最后一个材质槽的几何**(名字不变),其余槽各生成 `原名.001/.002/.003`,**每个新网格只留自己那一个材质槽**;所有部件沿用原对象的局部变换与 `matrix_parent_inverse` ⇒ 世界位姿逐位重合(实测 4 部件 `max|Δworld| = 0.000e+00`)
