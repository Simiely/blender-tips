...
> 📌 **文档基线**:2026-08-21(commit a5bdf65)输出路径与序列帧输出规范 v1.7.0
...
- **SINGLE_PROP 读自定义属性"脚本内改值不重算"(depsgraph 缓存,可能读到 0),UI 手动改正常;旧构建脚本(如 build_sky_sun_driver.py)会把 sun_elevation 驱动不断写回常量(如 `.1`=5.73°)导致高度无反应**:要脚本/UI 改值都立即生效,用**命名空间函数实时读 bpy**(如 `sky_sun_angle()` 读 天空控制["太阳角度"]),别用旧重构脚本重建(详见 docs/天空太阳高度驱动.md)
