...
- **关联效果「互相索引」标注**（用户要求："下次找就方便都找到"）
  - `docs/滚筒斜纹材质.md` 新增 **§0.6 关联效果**：一张表列出同一锚点下的全部材质
    （`_按条数` / `_按角度` / `_竖条旋转` / `_螺旋上升` / `_端盖黑` / `_自适应`），
    标注状态、实测 users、配套文档与脚本；并附**同名辨析**（避免改错材质）
  - 加**搜索关键词**行（滚筒 / 滚筒斜纹 / 竖条旋转 / 螺旋上升 / 圆柱条纹 / 理发店滚筒 …）
  - `scripts/streak-material/README.md` 加**名词对照表**；skill `description` 补全部关联材质名（便于触发）
- **修「贴到别的对象后没动态」**（实测两大真因，见 docs §5.5 / skill 坑 7d）：
  ① **尺度不匹配** —— 材质用世界坐标，贴到 scale≈0.01 的小球上（条纹周期 ≈7.9 世界单位）
     ⇒ 整块落在同一条纹内 ⇒ 渲染成纯色。修法：复制材质 + **清空 `纹理坐标.Object`** ⇒
     用对象局部坐标，尺度自适应（`make_adaptive_copy.py`）
  ② **相位步长恰为整数** —— 实测 `上升速度 0.2 × N 3 = 0.6 周期/帧`（1.7 帧一周期＝频闪），
     且 `Δframe×速度×N` 为整数时两帧**逐像素完全相同**（实测差 0.00000）。
     修法：每帧相位控制到 **0.01~0.05 周期** ⇒ 33 帧/周期流畅
- **新增判据**（skill 坑 7e）：判断"有没有动态"**别只看渲染** —— 隔离场景里驱动不自动求值，
  "渲染两帧做差"连正常在动的柱子都会测出 0 差异；**可靠做法是读求值依赖图里的驱动真值**
- 新增 4 个工具脚本：`diagnose_portable.py`（诊断锚点/驱动是否断）· `retarget_material.py`
  （断引用重接，**先检查后清空**）· `make_adaptive_copy.py`（尺度自适应副本）·
  `restore_legacy_angle.py`（补回旧版需要的 `斜角` 属性）
- 涉及文档：`docs/滚筒斜纹材质.md`、`scripts/streak-material/*`、
  `skills/blender-cylinder-spiral-material/*`、`CHANGELOG.md`、`AGENTS.md`
...
- **修「有一节对不上」**：实测根因是**端盖材质丢失** —— `material_slots` 只剩 1 个槽、
  全部 34 个面（含 2 个端盖）都指向条纹材质 ⇒ 端盖用柱面坐标投影摊成 K 个**扇形**。
  修法：建脚本重建端盖材质并按法线重新分配；核验改为断言「两个端盖面都指向端盖槽」。
- **整数类参数在驱动里取整**：`floor(kn + 0.5)` / `floor(hn + 0.5)`。
  实测用户拖滑块会拖出 `高度条纹数 = 2.5399999618530273`，`step=1` 挡不住；
  非整数 K ⇒ 绕柱接缝错位，非整数 N ⇒ 柱顶柱底错开。
  （顺带实测：`floor` / `round` / `int` 在 Blender 5.2 驱动表达式里都可用，`is_valid=True`）
- **槽位规范化为 `[0 条纹, 1 端盖]`**（原来是 1/2/3，且槽 0 曾是冗余副本）；
  两个方案的建脚本都改成 `SLOT_INDEX=0 / CAP_SLOT=1`。核验断言改为**按材质名定位、不写死索引**。
- 核验 **28 项全绿**；端盖实测俯视中心区最大亮度 `0.0000`（纯黑）。
- 涉及文档：`docs/滚筒斜纹材质.md`（新增坑 5.2b/5.2c）、`scripts/streak-material/*`、
  `skills/blender-cylinder-spiral-material/*`、`CHANGELOG.md`、`AGENTS.md`
...
- **跑哪个方案就只装哪个**：`build_streak_count.py` 增加 `cleanup_variant_b()`，
  装 A 时自动清掉 B 的材质、空槽位与它在 ctrl 上留下的参数键。
  理由：两方案共用同一个锚点空物体，并存会在 ctrl 上留下**对方用不到的幽灵参数**
  （典型：「斜角」在按条数方案里没有任何节点引用）。另一方的建脚本始终保留，随时可重装。
- 槽位回到 `[螺旋上升, 滚筒斜纹_按条数, 端盖黑]`；**7 个控件全部有功能**（各对应一条驱动）。
- `verify_variants.py` 改为**只验已装的方案**（未装自动 SKIP），并新增
  「幽灵参数检查」断言（ctrl 上不得有本方案用不到的键）与「槽位无空槽」断言。
