# build_cylinder_streak.py —— 「滚筒斜纹」发光材质 v3【在 v2 的 Math 域波形上增加螺旋项】
#
...
# ============================== 配置区 ==============================
TARGET_OBJ = '滚动效果网格体'
MAT_NAME = '滚动效果网格体_竖条旋转'      # 沿用同名 ⇒ 槽位不变，用户不用重新指派
CTRL_NAME = '竖条旋转控制'
SLOT_INDEX = 1
...
PANEL_SRC = r'D:\workbuddy\2026-09-20-09-39-42\_liantiao\streak_panel.py'
...
def cleanup_old_ui():
    ...
    for key, cls, txt in (('_streak_watch', 'VIEW3D_PT_streak', 'streak_panel.py'),
...
def main():
    ...
    try:
        ...
        _wf = bpy.app.driver_namespace.get('_streak_watch')
    ...
    peri = 2.0 * math.pi * 3.7789
    ang = math.degrees(math.atan((k / n) * HEIGHT / peri)) if n else 0.0
    print('=== BUILD_OK (v3 滚筒斜纹) ===')
    ...
    print('螺旋合成  : SUBTRACT | 条纹 %s×K − %s×N ⇒ %s 形，斜角 %.1f°'
