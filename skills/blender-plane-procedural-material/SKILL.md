...
- **改自定义属性本身不会让驱动重算**（实测：脚本赋值后立刻读仍是旧值）。
  ⇒ 控件要生效必须有 ①`SINGLE_PROP` 声明依赖边 ②不依赖 UI 的看门狗定时器。
  **只在面板 `draw()` 里 tag 不可靠** —— `draw()` 的执行时机不由你保证
  （面板没显示 / 没重绘就不跑）。
  ⚠️ **证据等级**：脚本侧不 tag 是实测；「**UI 里改数字**会不会自动 tag」仓库**未隔离实测**
  （`probe_driver_autotrigger.py` 用的是 Python 赋值；`setattr` 走不到 —— 自定义属性不在 `bl_rna`；
  `wm.properties_edit` 是 INVOKE-only 弹窗）。看门狗对两种情况都成立，照做即可。
