...
5. **跑「带还原逻辑」的技能脚本前，先用自己的隔离探针验它一遍**（2026-09-20 实测血泪）——
   技能的 `restore()` / `cleanup()` 未必真的只还原。做法：先建一个自己的临时对象当探针
   （如 2 槽 mesh、面索引交替 0/1）并记下它的状态，跑完脚本后**读探针确认它原封未动**，
   同时核对对象/材质总数与跑前是否一致。实测 `blender-radial-pulse-material/self_test.py`
   的旧 `restore()` 会 `me.materials.clear()` 把**全部 mesh** 的面级材质索引归零
   （探针 `{0:3,1:3}` → `{0:6}`；真实工程里 141 个 mesh 中招，单个最多 140 万面用槽 1），
   并且不删它自建的 `ctrl` 空物体。**别信脚本头部「跑完与基线逐字节等价」的自我声明 —— 那是声明，不是事实。**
6. 执行改动（脚本内做「三重闸 + 反向复核」）
7. **独立核验**（verify_*.py，全部用新取的引用）
8. 提醒用户：桥只改内存不存盘，满意 Ctrl+S，不满意 `File > Revert`
